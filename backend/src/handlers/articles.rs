...
pub async fn get_engine(pool: web::Data<Pool>, article_id: web::Path<i32>, model: web::Data<WordModel>) -> Result<HttpResponse, Error> {
    let mut connection = pool.get().unwrap();
    Ok(web::block(move || Article::get_engine_from_id(&mut connection, *article_id, &model))
        .await
